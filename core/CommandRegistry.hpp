...
public:
    ...
    Result<std::shared_ptr<IOutgoingMessageProvider> > getProviderInAuthSpace(const std::string& command) const;
    ...
    Result<std::shared_ptr<IExecutable> > getExecutorInAuthSpace(const std::string& command) const;
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<IOutgoingMessageProvider> > AuthSpaceProviders;
    ...
    std::unordered_map<std::string, std::shared_ptr<IExecutable> > AuthSpaceExecutors;
